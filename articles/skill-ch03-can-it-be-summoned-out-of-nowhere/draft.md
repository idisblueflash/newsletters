...
你有没有碰到，只说了一句普通的话，屏幕上却弹出了一个你没打算用的Skill？尤其是在你装了很多Skill之后。我装的Skill十个手指都能数得过来，所以我还没碰到，不过我们可以自己复现一下，看看屏幕上真的会弹出什么。

任何Skill创建的时候都会采用「预设」触发方式。也就是说，你可以通过 /skill-name手动触发它，Claude 也可以扫描你说的话，认出触发词，自动调用。

这就好比你去医院看病。如果你很清楚自己应该看哪科，你可以直接说「我要看牙科」（手动指定），但是有时候你只说了「我头疼」，分诊台的护士听完就把你领到神经内科门口，你甚至不用开口说「我要看哪科」。

「预设」触发的方式就是两个触发都开着，但有时候你没叫它，它自己跑出来了；或者你叫了一个，两个都举手。
...
>
> 如果你是在创建新的Skil，我推荐你不要这么做。直接claude命令进入。这样做，旧对话里的内容不会混进来，你能清楚地看到新Skill是不是真的被触发了。
...
刚才我们是用/anki-abstract-noun这个命令，主动触发的。你也可以直接说「生成Anki卡片」，Claude 认出这几个字，不问你，自己就调了：
```
生成Anki卡片
```
# 动词卡片skill
...
输入`制作Anki卡片`，屏幕上弹出来一个：anki-abstract-noun。anki-verb 没有动。
...
# 触发率
触发率决定了你是直接看到结果，还是盯着屏幕上的反问发愣。就像上面那个例子，Claude会问你到底想用哪个？你说了一句话，它停下来反问你，你还要再回答一次，才能真正开始干活。最好是说完那句话，屏幕上直接弹出结果，一个字都不用多说。

有人做了650次受控实验，专门测description的写法。同样的触发词，换一种写法，触发率从37%跳到94%。也就是说，用被动式写法，发出三次只有一次真的触发了skill，另外两次只输出了普通文字，skill图标一次都没弹；换成指令式，十次里有九次它自己就认出来了。
...
被动式触发率低，是因为它只描述了用途，没有对 Claude 发出明确的行动指令。Claude 读完，没动，然后问你一句。你说完那句话，盯着屏幕等了两秒，出来的不是结果，而是一个反问。
