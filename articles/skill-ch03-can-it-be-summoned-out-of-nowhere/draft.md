...
你有没有碰到Skill会胡乱触发的情况？尤其是在你装了很多Skill之后。我装的Skill十个手指都能数得过来，所以我还没碰到，不过我们可以自己复现一下，看看屏幕上真的会弹出什么。
...
刚才我们是用/anki-abstract-noun这个命令，主动触发的。你也可以直接说「生成Anki卡片」，Claude认出这几个字，就会自己找到该用哪个skill：
```
生成Anki卡片
```
# 动词卡片skill
...
# 触发率
更重要的是触发率。就像上面那个例子，Claude会问你到底想用哪个？你说了一句话，它停下来反问你，你还要再回答一次，才能真正开始干活。最好是我们说了，它自己认出来，直接调用，不问你一句。

有人做了650次受控实验，专门测description的写法。结论是：同样的触发词，换一种写法，触发率能从37%跳到94%。也就是说，用被动式写法，发出三次只有一次真的触发了skill，另外两次Claude就那么晾着你；换成指令式，十次里有九次它自己就认出来了。
...
被动式触发率低，是因为它只描述了用途，没有对 Claude 发出明确的行动指令。Claude 读完，不确定该不该出手，就停下来问你一句。你说完那句话，盯着屏幕等了两秒，出来的不是结果，而是一个反问。
