...
这就好比你去医院看病。如果你很清楚自己应该看哪科，你可以直接说「我要看牙科」（手动指定），但是有时候你只说了「我头疼」，分诊台的护士听完就把你指向神经内科，你不需要知道这个分法从何而来。

「预设」触发的方式就是两个触发都开着，但是你可能会碰到 skill 胡乱触发，或者几个 skill 争着要响应的情况。
...
那么如果我们把同一个触发短语也加进anki-verb，能不能让两个skill同时抢着响应？我把同一个触发短语加进去，发了同样一条消息，等来的不是随机的跳出来一个skill，而是 Claude 停下来问你想用哪个。
```
⏺ 请问你想生成哪类 Anki 卡片？
...
# 触发率
更重要的是触发率。就像上面那个例子，Claude会问你到底想用哪个？你说了一句话，它停下来反问你，你还要再回答一次，才能真正开始干活。最好是我们说了，它就自动找到正确的skill。

有人做了650次受控实验，专门测description的写法。结论是：同样的触发词，换一种写法，触发率能从37%跳到94%。
...
被动式触发率低的原因在于，它只描述了用途，没有对 Claude 发出明确的行动指令。Claude 读到那句话，不知道该不该动，只好停下来问你。
