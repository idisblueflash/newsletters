...
你有没有碰到，说了一句「帮我生成卡片」，屏幕上却弹出了一个你没打算用的Skill？尤其是在你装了很多Skill之后。我装的Skill十个手指都能数得过来，所以我还没碰到，不过我们可以自己复现一下，看看屏幕上真的会弹出什么。
...
650次测试证明：同样的触发词，换一种写法，触发率从37%跳到94%。也就是说，用被动式写法，发出三次只有一次真的触发了skill，另外两次只输出了普通文字，skill图标一次都没弹；换成指令式，十次里有九次它自己就认出来了。
