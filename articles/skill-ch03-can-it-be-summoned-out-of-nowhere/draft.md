...
任何Skill创建的时候都会采用「预设」触发方式。也就是说，你可以通过 /skill-name手动触发它，你的Claude也可以根据聊天的意思，自动触发。
...
```
针对上面的这段，我们就可以设计一个Skill来制作Anki卡片。

Anki卡片是由正反面两部分组成， 考核的词， 和答案。答案里包括理由。大概是这个结构：
...
Claude调用的时候，会看出问题，然后找你确认，而不是上来就直接套用。

所以这个胡乱触发的情况不会每次都出现的，或者说很少会出现。

# 触发率
更重要的是触发率。就像上面那个例子，Claude会问你到底想用哪个？这样肯定不行。最好是我们说了，它就自动找到正确的skill。

在650次受控实验中发现，description的格式对触发率影响比大家想象的还要大。
