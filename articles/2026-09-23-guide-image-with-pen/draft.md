...
我把提示词准备好，一段管风格，另一段管内容。然后一起给到 ComfyUI，但是它生成的图片构图不对，我想生成的是：两条平行又稍微倾斜的电线上三三两两地落着一些燕子：
...
图片看起来是不是很干净？因为我用了绘图板。但是不用也行。我们在一张纸上画出来也可以的。因为这里的 ControlNet 关心的是图片中的轮廓（Canny 模式）：

![「图：手绘构图原图与 Canny 边缘检测提取出的轮廓图对比」](assets/pen-guide-contour-comparison.png)

「图：原图和轮廓图对比」

所以不管是在格子本上，还是发黄的病历上画的，它都不会介意。而且也不用画得太精细。我用曲线表示电线，粗点示意燕子们的位置。
...
把你喜欢的构图用笔简单画出来，给 ComfyUI 做 ControlNet 的参考。它出来的图片就能更好的还原你的设计。
