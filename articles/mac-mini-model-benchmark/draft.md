# Mac Mini 本地模型测试

上次我在配置多个模型的时候，已经让 Claude 顺手把 workflow 都配完了。这要是放在以前，我早就开始测试了。不过这次我耐着性子先让 Claude 出了一个测试计划。

我把大概测试目的：品质和速度提了一下。重点提醒 Claude 模型第一次有加载时间(cold）所以要多生两次来记录没有加载模型的速度（warm）。

Claude 生成了一份仔细的计划，连多个模型之间的接力顺序都想好了，挺不错的。我让他把这份计划存成 markdown 文件，然后清空对话框，让 Claude 开始测试。

其实我不知道他跑完所有测试用了多久。四个模型每个三张有 12 多张要生成。我后面去理发了，回来的时候它已经完成了，还有一份报告。

我有一个叫 _tracy 的 subagent 是平时跑测试用的。也许 Claude 看了 _tracy 的文档。所以他自动生成了报告文档，还把生成的图片放了进去。我看起来很方便。

报告的评分是这样的：

模型名称，单张速度，品质，年份
Juggernaut SD1.5, 115s, 8, 2023
Z-Image-Turbo, 122s, 9, 2025
Flux Schnell, 81s, 8, 2024
SD3.5 Medium, 179s, 7, 2024

Juggernaut 是 baseline，所以先不用看。剩下来是 SD3.5 又慢又差，也排除掉。再剩下来是 Z-Image-Turbo 首选，因为我对图片质量要求高。Flux Schnell 可以用来生成草稿。

这次生图的主题是机器人的，我平时的图是有人物和动作的。所以我又让 Claude 再跑了一次。这次换成了印第安纳·琼斯的动作图。等我散步回来，报告就出来了：

模型名称，单张速度，品质，年份
Juggernaut SD1.5, 113s, 5
Z-Image-Turbo, 123s, 8
Flux Schnell, 81s, 7
SD3.5 Medium, 176s, 7

从机器人的图上，我只能看出结构是不是够精细，而我完全信任 Claude 的评分了。这次有些不同，人物动作的图片看一眼就能分出好坏。

这两组测试下来，我已经知道 Z-Image-Turbo 是最后的选择了，哪怕会慢一点。
