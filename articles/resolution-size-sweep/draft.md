...
不过计时口径要说清楚：下面每个数字都只算纯粹的生图时间，切换模型的时间和 ComfyUI 各节点的调度开销都没算进去，这样才能拿三个模型的数字直接比较。
...
有时候提高尺寸可以让图片变得更好，但这也是要看情况的。生图模型训练的时候会用固定的尺寸，这个尺寸生图才保险，低于或者高于这个尺寸都会出问题。

拿这次测试来说，三个模型放到 1280x1280 时都出现了同一类瑕疵：画面里多出一个前景小物件。Z-Image 多了一个墨水瓶，Flux Schnell 多了一个雕像头，Klein 多了一个不知从哪来的小饰品。
